		...
		<table cellpadding='0' cellspacing='0'  border='0' class='display' id='tblCoupons' width='100%'>
			...
			while($row = mssql_fetch_array($query))
			{
				echo "<tr style='font: Verdana; font-size:11px; height:25px;' class='gradeU'  align='center'>
					...
					{
						echo '<span class="ui-icon ui-icon-tag" style="cursor:pointer" title="Tag Redeemed" onclick="tagRedeemed(\''.$row['CSTUBNO'].'\',\''.$row['ILOCATIONID'].'\');"></span>';
...
<script language="javascript" type="text/javascript">
	...
	function tagRedeemed(stub, str)
	{
		...
		if(v_store == u_store)
		{
			...
			{
				$.ajax({
					url:'employeecoupons_trs.php?action=redeem_stub',
					data:{stub:stub},
					method:'post',
					type:'POST',
					dataType:'json',
