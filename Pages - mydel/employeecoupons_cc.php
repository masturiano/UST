<?php 
		...
		<table cellpadding='0' cellspacing='0'  border='0' class='display' id='tblCoupons' width='100%'>
			...
			while($row = mssql_fetch_array($query))
			{
				echo "<tr style='font: Verdana; font-size:11px; height:25px;' class='gradeU'  align='center'>
					...
					{
						echo '<span class="ui-icon ui-icon-circle-check" title="Received"></span>';  
					}
					else
					{
	...
	else if(isset($_GET['action']) && $_GET['action'] == 'receive_stubs')
	{
		...
		else
		{
			...
			// $res_soano = mssql_fetch_array($qryGetSOA);", SOANO=" . $res_soano['soano'] . 
			...
			if($qryUpdate)
			{
				...
				// if($qryUpdateSOA),'soano'=>$res_soano['soano'],'dreceived_from'=>$date_from,'dreceived_to'=>$date_to
				// {
					$datetime = date('Y-m-d h:i:s', time() + 28800);
					$strLogAction = "INSERT INTO tblaudittrail(cUserID, process_date, process) VALUES('".$_SESSION['session_id']."','".$datetime."','TAGGED RECEIVED STUB NUMBERS: ".$stub."') ";
					mssql_query($strLogAction);
					echo json_encode(array('success'=>true, 'msg'=>'PG-UST Coupons successfully tagged as received.'));
				// }
				// else
				// {
	...
	else if(isset($_GET['action']) && $_GET['action'] == 'untag_stubs')
	{
		...
		{		
			...
			{
				$datetime = date('Y-m-d h:i:s', time() + 28800);
				$strLogAction = "INSERT INTO tblaudittrail(cUserID, process_date, process) VALUES('".$_SESSION['session_id']."','".$datetime."','UNTAGGED REDEEMED STUB NUMBER: ".$stub."') ";
				mssql_query($strLogAction);
