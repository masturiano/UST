...
while($company = mssql_fetch_array($result_company))
{
	...
	$i++;
	$worksheet->write($i,0,'COVERED PERIOD : ' . $from . ' - ' . $to,$headerFormat);
	$worksheet->mergeCells($i,0,$i,4);
	...
	$strSQL = "SELECT iLocationID, cStoreDesc, CONVERT(varchar,dRedeemed,101) AS dRedeemed, cEmployeeName, cStubNo, mAmount
