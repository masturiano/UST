...
$strSQLCompany = "SELECT DISTINCT case when compcode = 3 then '1' else compcode end as compcode, compname 
    FROM view_coupons
    WHERE soano = " . $soano;
...
class PDF extends FPDF
{
	...
	function Main($arrH)
	{
        ...
        End of comment*/	
            ...
            $qryResult = $this->getDetails($this->soa_no);
            //$dueAmt = $this->getDueAmt($this->soa_no, $company['compcode']);
            $dueAmt = $this->getDueAmt($this->soa_no);

			$this->SetFont('Arial','B',15);
			//$this->Cell(98,10, $company['compname'],0,0,'L');
            $this->Cell(98,10, 'PUREGOLD PRICE CLUB, INC',0,0,'L');
			...
			{	
				...
				$this->SetXY(12, $y);
				$this->Cell(50,4, $this->date_from . "-" . $this->date_to,0);
				$this->SetXY(67, $y);
    ...
    function getDetails($soano)
	{
		/* Comment by Mydel
        $strSQL = " SELECT noOfStubs, compcode, compname, soano, iLocationID, cStoreName, totalAmt
                    FROM view_coupons
        ...
        $strSQL = " SELECT noOfStubs, compcode, compname, soano, iLocationID, cStoreName, totalAmt
                    FROM view_coupons
                    WHERE soano = ". $soano . " AND compcode in (1,3)";
        $qrySQL = mssql_query($strSQL);
		return $qrySQL;
	}
	
	//function getDueAmt($soano, $compcode)
    function getDueAmt($soano)
	{
		/* Comment by Mydel
